...
- 在保持原版浅色、克制、建筑感视觉系统不变的前提下，补齐与 **SCULP After Dark** 一致的交互层：滚动揭示、页面淡入、顶部阅读进度线、导航下划线动画、卡片与按钮反馈、移动菜单过渡。
- 为 Portfolio 真实图片加入全屏灯箱、上一张/下一张、图片计数、键盘方向键、Esc 关闭、焦点循环与桌面 VIEW 跟随光标；当前占位图不触发灯箱，后续补入真实图片后自动生效。
- 完整支持系统“减少动态效果”偏好；未修改原有页面结构、文案、链接、表单字段、颜色、字体及内容占位。
